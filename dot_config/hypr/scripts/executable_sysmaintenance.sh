#!/usr/bin/env bash
# ------------------------------------------------------------
# Copied from: https://github.com/xZepyx/HyprZepyx/blob/main/scripts/kernel-fault/sysmaintenance.sh
# ------------------------------------------------------------
# Arch "Clean" Maintenance Script
...
cache_before=$(sudo du -sh ~/.cache | cut -f1)
...
cache_after=$(sudo du -sh ~/.cache | cut -f1)
...
announce "Clean finished in ${SECONDS}s — log saved to $LOG_FILE"
