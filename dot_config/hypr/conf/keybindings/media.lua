...
local volume_gesture = function(change)
    hl.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ " ..
        math.abs(change) .. "%" .. (change < 0 and "-" or "+") .. " && " .. volumeNotify())
end
hl.gesture({
    fingers = 4,
    direction = "vertical",
    ...
    action = {
        start = function(e) volume_gesture(-0.05 * e.delta.y) end,
        update = function(e) volume_gesture(-0.05 * e.delta.y) end
    }
})
...
local function playNextPrev(mod, key, cmd)
    BindExec(mod, key, "playerctl " .. cmd, { locked = true })
end
